...
//String APIROOT = "http://raspberrypi.local:8086/";
String APIROOT = "http://192.168.0.3:8086/"; // Direct IP
void setup(){
  Serial.begin(9600);
...
void loop(){
  ...
  // create payload
  String tags = "id=" + String(ESP.getChipId(),HEX);
  ...
  String POSTURL = APIROOT + "api/v2/write?org=HouseSlattery&bucket=SlattAirQ&precision=s";
  Serial.println(POSTURL);

  // Hack to not check finger prints on https connection
  // Not using currently for raspberry pi
  ...
  http.addHeader("Authorization", "Token N6z4RVHp6Ccn_VkJyR502Uludwy1BTZ75SqBOuCqVRChYt2P4TOj74SVwkcwTDs-VV0iGYSACELzb3VEJY_pLg==");
